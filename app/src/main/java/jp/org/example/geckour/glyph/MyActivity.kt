...
//                canvas.drawText(calcBonus(level, true, correctNum.toLong(), upTime) + "%", offsetX, offsetY * 2 - 80 * scale, paint)
        ...
        override fun onTouchEvent(event: MotionEvent): Boolean {
            when (event.action) {
                ...
                -> {
                    if(isEndGame) {
                        startActivity(Intent(this@MyActivity, FortuneActivity::class.java))
                    }
                }
            }
        ...
        fun drawButton(canvas: Canvas) {
            if(isEndGame) {
                ...
                canvas.drawText("NEW FORTUNE", (nextButtonPoint[0]?.x ?: 0) + nextButtonWidth / 2, (nextButtonPoint[1]?.y ?: 0) - buttonBaseline * scale, paint)
